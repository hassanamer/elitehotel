...
class _GuestScreenState extends State<GuestScreen> {
  ...
  Future<void> fetchGuestData() async {
    try {
      ...
      List<Map<String, dynamic>> roomsData = roomSnapshot.docs.map((doc) => doc.data() as Map<String, dynamic>).toList();
      ...
      setState(() {
        guestData = reservationSnapshot.docs.map((doc) {
          ...
          DateTime? checkInDate = (reservationData['checkInDate'] as Timestamp).toDate();
          DateTime? checkOutDate = (reservationData['checkOutDate'] as Timestamp).toDate();
          ...
          } else if (today.isAtSameMomentAs(checkInDate) || (today.isAfter(checkInDate) && today.isBefore(checkOutDate))) {
  ...
  void filterData() {
    ...
    setState(() {
      filteredData = guestData.where((guest) {
        bool matchesStatus = selectedStatus == 'All' || guest['checkInOut'] == selectedStatus;
        bool matchesSearch = (guest['name']?.toString().toLowerCase().contains(searchQuery) ?? false) ||
            (guest['reservationId']?.toString().toLowerCase().contains(searchQuery) ?? false) ||
            (guest['roomNumber']?.toString().toLowerCase().contains(searchQuery) ?? false);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Guest Management', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
